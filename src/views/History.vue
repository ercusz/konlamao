<template>
    ...
    <!-- Confirm Modal -->
    <div class="modal fade" id="confirmPaymentMethod" tabindex="-1" aria-labelledby="confirmPaymentMethodLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content bg-dark text-white">
          <div class="modal-header bg-dark text-white">
            <h5 class="modal-title" id="confirmPaymentMethodLabel">ต้องการส่งหลักฐานการโอนเงินของ การจองหมายเลข {{this.currentConfirm.id}} ใช่หรือไม่?</h5>
          ...
          <div class="modal-body bg-dark text-white">
            ...
            <div class="mb-3">
              ...
              ชื่อบัญชี: <strong class="text-info">บริษัท คนละเมา จำกัด</strong><br>
              พร้อมเพย์: <strong class="text-info">098-888-6888</strong><br>
              ธนาคารกสิกรไทย: <strong class="text-info">035-0-12345-6</strong><br>
              ธนาคารไทยพาณิชย์: <strong class="text-info">049-3-45678-9</strong><br>
            </div>
            ...
            <div>
              ...
              <form>
                ...
                <select v-model="this.selectedPaymentMethod" class="form-select" id="selectPaymentMethod">
                  <option selected>--กรุณาเลือกช่องทางการชำระเงิน--</option>
                  <option v-bind:value="pm" v-for="pm in paymentMethod" :key="pm">{{pm}}</option>
                </select>
                <label for="selectCustomerBank">บัญชีธนาคารของลูกค้า</label>
                <select v-model="this.selectedCustomerBank" class="form-select" id="selectCustomerBank">
                  <option selected>--กรุณาเลือกบัญชีธนาคารของลูกค้า--</option>
                  <option v-bind:value="cb" v-for="cb in customerBank" :key="cb">{{cb}}</option>
                </select>
                ...
                <div class="input-group" id="paymentTime">                  
                  <input type="text" class="form-control" placeholder="ชั่วโมง" aria-label="Hours" v-model="paymentHours">
                  <span class="input-group-text">:</span>
                  <input type="text" class="form-control" placeholder="นาที" aria-label="Minutes" v-model="paymentMinutes">
                </div>
...
export default {
  ...
  data() {
    return {
      ...
      customerBank: [ "ธนาคารกสิกรไทย", "ธนาคารไทยพาณิชย์", "ธนาคารกรุงไทย", "ธนาคารกรุงเทพ", "ธนาคารกรุงศรีอยุธยา" ],
  ...
  methods: {
    sendPaymentDesc(){
      ...
      */
      if(this.selectedPaymentMethod != "--กรุณาเลือกช่องทางการชำระเงิน--" 
      && this.selectedCustomerBank != "--กรุณาเลือกบัญชีธนาคารของลูกค้า--"
      && this.paymentMoney > 0 && this.paymentDate != null
      && this.paymentHours >= 0 && this.paymentHours <= 24
      && this.paymentMinutes >= 0 && this.paymentMinutes <= 60
      ){        
        ...
        .set({
          ...
          paymentDate: new Date(new Date(this.paymentDate).setHours(this.paymentHours, this.paymentMinutes))
        })
        .then(() => {
          db.collection("invoices").doc(this.currentConfirm.id)
          .update({'status': db.doc('invoiceStatus/' + '4')}).then(() => {
            db.collection("Tables").doc(this.currentConfirm.table)
            .update({'available': true})
            .then(() => {
              alert("ส่งหลักฐานการชำระเงินเรียบร้อย กรุณารอการยืนยันจากระบบ")
    ...
    cancelReserve(invoice){
      ...
      .update({'status': db.doc('invoiceStatus/' + '2')}).then(() => {
            db.collection("Tables").doc(invoice.table)
            .update({'available': true})
            .then(() => {
